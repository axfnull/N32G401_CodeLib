...
void PWR_Wakeup_Pin_Enable(WAKEUP_PINX pin, uint32_t polarity)
{
    ...
    {
        case WAKEUP_PIN:
            *(__IO uint32_t*)PWR_CTRLSTS_WKUPPS_BITBAND = polarity;
            *(__IO uint32_t*)PWR_CTRLSTS_WKUPEN_BITBAND = (uint32_t)ENABLE;
            break;
        default:
            break;
    }
...
void PWR_Wakeup_Pin_Disable(WAKEUP_PINX pin)
{
    ...
    {
        case WAKEUP_PIN:
            *(__IO uint32_t*)PWR_CTRLSTS_WKUPEN_BITBAND = (uint32_t)DISABLE;
            break;
        default:
            break;
    }
