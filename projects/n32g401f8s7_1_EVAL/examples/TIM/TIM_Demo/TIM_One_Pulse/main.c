...
int main(void)
{
    ...
    /* GPTIM configuration: One Pulse mode 
     ...
     TIM3CLK = GPTIMClockFrequency, we want to get TIM3 counter clock at 9 MHz:
     - Prescaler = (TIM3CLK / TIM3 counter clock) - 1
     The Autoreload value is 65535 (GPTIM->AR), so the maximum frequency value
     to trigger the GPTIM input is 9000000/65535 = 137HZ
     One Pulse value = (Period - Pulse) / GPTIM counter clock = 5.46 ms.

                                                                 */

    PrescalerValue = (uint16_t)(GPTIMClockFrequency / 9000000) - 1;
