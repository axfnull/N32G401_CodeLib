...
int main(void)
{
    ...
    while(1)
    {
        if(GPIO_Input_Pin_Data_Get(KEY3_PORT, KEY3_PIN) == PIN_RESET)
        {
            ...
            /* Disable the SW-DP Debug Port  */
            SWD_As_GPIO_Initialize();
            
            while(1)
            {
                /* Toggle SWDIO pin */
                if(GPIO_Input_Pin_Data_Get(SWDIO_PORT, SWDIO_PIN) == RESET)
                {
                ...
                /* Toggle SWDCLK pin */
                if(GPIO_Input_Pin_Data_Get(SWDCLK_PORT, SWDCLK_PIN) == RESET)
                {
                    GPIO_Pins_Set(SWDCLK_PORT, SWDCLK_PIN);
                }
                else 
                {
                    GPIO_PBC_Pins_Reset(SWDCLK_PORT, SWDCLK_PIN);
                }
                /* Insert delay */
                Delay(0x8FFFF);
